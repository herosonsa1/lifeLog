...
class SyncHistoricalDataUseCase @Inject constructor(
    ...
    operator fun invoke(context: Context, daysBack: Int? = null): Flow<SyncProgress> = flow {
        try {
            ...
            // 라커룸 안내지 사진 자동 OCR 감지 (1달치 동기화 시 과부하/OOM 방지를 위해 최대 2장 제한)
            ...
            for (photo in golfCandidates) {
                try {
                    // [L-02] Uri.parse() 전 빈값 방어 — NPE/IllegalArgumentException 방지
            ...
            emit(
                SyncProgress(
                    isRunning = false,
                    stage = "동기화 완료: 결제 ${txCount}건, 사진 ${photoCount}장 색인 및 다이어리 ${insertedEntriesCount}일 구축 완료!",
