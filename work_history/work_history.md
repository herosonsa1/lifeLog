...
## 54. 출퇴근 주행 자동 트래킹 시스템 및 가계부-차계부 주유 연동·실연비 산출 실증

- **일시**: 2026-09-21
- **목적 및 배경**:
  - 출퇴근 왕복 주행이 차계부에 자동으로 기록되는지 여부 및 동작 메커니즘 확인 요청.
  - 가계부의 주유비 결제 내역이 차계부에 연동되는지 여부 및 표시 방식 검증 요청.

### 54.1. 주요 분석 및 아키텍처 구조
1. **출퇴근 자동 주행 트래킹 메커니즘 (`CarDrivingTrackingService.kt`)**:
   - **차량 블루투스 연결/해제 기반 감지**: 스마트폰이 차량 블루투스에 연결되면 백그라운드 주행 추적 서비스가 자동 시작되고, 연결 해제 시 주행이 자동 종료됨.
   - **거점(집/회사 반경 800m) 지능형 판별**:
     - 통짜 "왕복 1건"이 아닌, 실제 일과 생활 패턴에 맞춰 **오전 '출근(편도)' 1건 + 저녁 '퇴근(편도)' 1건**으로 분리 실시간 자동 기록.
     - 출발지가 집이고 도착지가 회사이면 `"출근 (회사 가는 길)"`, 출발지가 회사이고 도착지가 집이면 `"퇴근 (집 가는 길)"`로 명확히 라벨링.
   - **실제 주행거리 보정 및 가이드**: 직선거리 대비 도로 굴곡률(1.25배)을 반영하여 현실적인 편도/왕복 주행거리를 자동 산출. 원활한 자동 판별을 위해 설정 화면 내 "우리집 & 회사 거점 등록" 가이드 정립.

2. **가계부 결제-차계부 주유 기록 지능형 자동 연동 (`VehicleRepositoryImpl.syncRefuelingFromTransactions`)**:
   - **SMS/앱 푸시 자동 감지**: 카드사 승인 문자/푸시 알림 수신 시 가계부 `transactions` 테이블에 인입된 결제처(GS칼텍스, SK에너지, 현대오일뱅크, S-OIL 등)를 지능형 필터링하여 차계부 `vehicle_logs`의 `REFUELING` 타입으로 실시간 변환.
   - **주유량 및 실연비 자동 산출 엔진**:
     - 결제 금액을 전국 평균 유가(기본 1,650원/L)로 역산하여 주유량(L) 자동 산정.
     - 직전 주유 시점 이후 누적된 주행거리를 자동 집계하여 **실연비(km/L)** 자동 계산 (`누적 주행거리 ÷ 주유량`).
   - **사용자 수정 UI 지원**: 차계부 화면 내 주유 카드 우측 연필 아이콘을 통해 실제 영수증 리터(L)로 즉시 미세 조정 가능.

### 54.2. 실기기(에뮬레이터) 검증 내역
- **가계부 결제 데이터 주입 및 동기화 테스트**:
  - 에뮬레이터 DB의 `transactions` 테이블에 주유 결제 레코드(GS칼텍스 판교주유소 75,000원, SK에너지 60,000원) 주입.
  - 차계부 화면에서 `[주유 동기화]` 구동 및 실시간 렌더링 확인.
- **에뮬레이터 실화면 렌더링 검증 완료**:
  - `GS칼텍스 판교주유소 75,000원 (45.5 L)` 자동 변환 표출.
  - 직전 주유(SK에너지 60,000원, 36.4 L) 이후 누적 주행거리 `이전 주유 후 290.0 km`, 실연비 `6.4 km/L` 산출 표출 확인.
  - 상단 차량 요약 카드 내 `총 주유비 135,000원` 파란색 카운터 정상 실시간 반영 확인 완료 (`screen_car_refuel.png`, `screen_car_refuel2.png`).

---

## 55. 다이어리 타이틀·동선·주행거리 결함 정밀 분석 및 자가 치유(Self-Healing) 엔진 구축

- **일시**: 2026-09-22
- **사용자 제보 및 배경**:
  - "타이틀도 이상하고, 타임라인도 이상하고, 이동거리도 이상해"
  - 첨부 이미지 분석 결과:
    1. **타이틀 왜곡**: `"라운드 상세 기록 FIELD / 기록 삭제 취소 저장하기 CC 일정"`
    2. **타임라인/동선 왜곡**: 대표 사진은 가족 일상 사진(문정동)인데 골프 일정이 덮어씌워지고, 이동 동선 요약 체인에 위치 없는 플레이스홀더인 `"사진 기록"`이 들어가 `"사진 기록 ➔ 서울 문정동"`으로 노출됨.
    3. **이동거리 결함**: 골프장 라운드가 존재함에도 차계부/GPS 주행거리가 `0.0 km`로 표시됨.

### 55.1. 결함 근본 원인 분석
1. **스코어카드 OCR 파서의 모바일 앱 UI 키워드 오탐**:
   - 모바일 골프 앱(스마트스코어 등) 스크린샷 상하단에 위치한 UI 버튼("라운드 상세 기록 FIELD", "기록 삭제 취소 저장하기")이 `ScorecardOcrAnalyzer.extractCourseGrid`에서 코스명(`courseName`) 후보로 오인식됨.
   - `ExtractScorecardOcrUseCase`에서 추출된 코스명 뒤에 무조건 `"${courseName} CC"`를 접미사로 붙여 `"라운드 상세 기록 FIELD / 기록 삭제 취소 저장하기 CC"`라는 오염된 골프장명이 탄생함.
2. **동선 체인 내 더미 장소("사진 기록") 유입**:
   - `DailyRouteAggregator`에서 위치 메타데이터가 없는 사진을 "사진 기록"으로 기본 라벨링하였으나, `distinctPlaces` 집계 시 이 문자열을 필터링하지 않아 동선 요약(`movementSummary`)에 `"사진 기록 ➔ 서울 문정동"` 형태로 노출됨.
3. **골프장 표준 왕복거리 연동 부재**:
   - 골프 라운드가 확정되었으나 차계부 GPS 기록이 없는 경우, 다이어리 집계 시 주행거리가 0.0km로 방치되어 현실의 100km 이상 이동 거리와 극심한 괴리가 발생함.

### 55.2. 주요 개선 및 구현 내역
1. **모바일 앱 UI 키워드 차단 블랙리스트 및 골프장명 살균 (`ScorecardOcrAnalyzer.kt`, `ExtractScorecardOcrUseCase.kt`)**:
   - `ScorecardOcrAnalyzer`에 `MOBILE_APP_UI_KEYWORDS`("기록 삭제", "취소", "저장하기", "FIELD", "라운드 상세", "코스선택", "스코어입력" 등)을 정의하고, 코스명 및 구장명 탐색 루프에서 원천 제외.
   - `ExtractScorecardOcrUseCase`에 `sanitizeClubName()` 가드레일을 구축하여 UI 키워드 포함 문자열에 CC/GC가 덧붙여지는 것을 차단하고, 실제 등록된 골프장 명칭으로 정제.
   - 티오프 시간(`extractTeeOffTime()`) 자동 추출기를 탑재하여 스코어카드 상의 06:20 티타임을 다이어리 라운드 시간으로 현실화.
2. **다이어리 동선 체인 정제 및 골프장 표준거리 자동 연동 (`DailyRouteAggregator.kt`)**:
   - `invalidPlaceKeywords`에 `"사진 기록"` 및 `"사진 촬영"`을 추가하여 `distinctPlaces`와 `movementSummary`에서 배제, 실제 유효 방문지(예: `킹스데일 GC (동 코스)`, `서울 문정동`)만 동선으로 연결되도록 보장.
   - `isRealGolfClub()` 가드레일을 대폭 강화하여 UI 키워드가 포함된 가짜 골프장명을 원천 탈락시킴.
   - 골프 라운드가 존재하고 GPS/차계부 거리가 0km인 경우, `GolfCourseDistanceUtils.getEstimatedRoundTripKm`를 통해 수도권 기준 표준 왕복 거리(120~160km, 킹스데일 GC 160.0km)를 다이어리 총 주행거리에 자동 연동.
   - 골프장 일정과 일상 장소(문정동 등)가 함께 있을 경우 `"킹스데일 GC & 서울 문정동 일정"`과 같이 조화로운 타이틀을 생성하도록 결합 로직 복원.
3. **다이어리 데이터 자가 치유(Self-Healing) 파이프라인 구축 (`DiaryRepositoryImpl.kt`, `DiaryViewModel.kt`)**:
   - `DiaryRepositoryImpl`에서 오염된 레거시 타이틀("기록 삭제", "FIELD", "사진 기록 일정" 등)이 감지되면 자동으로 최신 정제 파이프라인을 거쳐 정규 타이틀로 자가 치유하도록 구현.
   - `DiaryViewModel`의 `observeData()` 및 `checkAndUpgradeLegacyEntries()`에서 화면 진입 시 오염된 타이틀, "사진 기록" 동선 체인, 0.0km 골프 주행거리를 실시간 보정하고 DB에 최신 집계값으로 영속 업데이트.
4. **신규 안티패턴 등록 (`anti_patterns.json`)**:
   - `AP-OCR-MOBILE-APP-UI-TEXT-CORRUPTION` 등록 완료 (승인 케스케이드 및 영구 지식화).

### 55.3. 단위 테스트 및 실기기(에뮬레이터) 검증
- **단위 테스트 100% 통과**:
  - `ScorecardOcrAnalyzerTest`: 모바일 UI 텍스트("FIELD", "기록 삭제 취소 저장하기") 입력 시 코스명/구장명 오염 방지 검증 통과.
  - `DailyRouteAggregatorTest`: UI 키워드 가짜 골프장 배제, "사진 기록" 동선 체인 배제, 주행거리 0km 시 골프장 160km 자동 폴백, 골프+문정동 조화 타이틀 생성 검증 4개 케이스 전체 통과 (`BUILD SUCCESSFUL in 1m`).
- **에뮬레이터 실화면 렌더링 검증 완료**:
  - 앱 실행 및 다이어리 타임라인 화면 확인 (`screen_verify2.png`, `screen_detail2.png`):
    - **타이틀**: `"2026년 9월 21일 (월) 킹스데일 GC 라운딩"` (UI 오탐 완전 제거 및 정상 구장명 복원)
    - **주행거리**: `"160.0 km"` (현실적인 골프장 왕복 거리 자동 반영)
    - **이동 동선**: `"킹스데일 GC (동 코스)"` ("사진 기록" 잔재 완벽 제거)
    - **골프 상세**: 티오프 06:20, 81타, 퍼트 34P, 버디 3·파 5·더블+ 2, 벌타 3타 등 완벽 표출 확인.

---

## 56. 임의 바운딩박스 하드코딩 전면 제거 및 사진 EXIF GPS 기반 Android Geocoder 1순위 위치 판별 엔진 구축

- **일시**: 2026-09-22
- **사용자 제보 및 배경**:
  - "사진의 메타정보에서는 '서울 문정동'이 아닌데, 왜 문정동 일정으로 표기돼?"
  - 사용자 지침: "당연하지. 하드코딩하지말고, 메타데이터를 기반으로 기록되어야해"
  - 첨부 이미지 분석 결과:
    - 갤럭시 갤러리 사진(`20260921_201856.jpg`) 상세정보의 Google Map 위치는 **"대한민국 경기도 하남시"**인데, 라이프로그 앱 다이어리에서는 **"서울 문정동 일정"**으로 잘못 표기됨.

### 56.1. 결함 근본 원인 분석
1. **임의의 직사각형 바운딩 박스 하드코딩에 의한 인접 지자체 침범**:
   - `PlaceResolver.kt`에 송파구 바운딩 박스(`lat in 37.480..37.535 && lng in 127.080..127.160`)가 단순 직사각형 위경도로 하드코딩되어 있었음.
   - 송파구 동부와 맞닿아 있는 경기도 하남시 서부(감일동, 학암동, 위례 북부)가 이 사각형 영역에 침범되었고, `lat < 37.500` 분기 조건으로 인해 실제 행정구역이 경기도 하남시임에도 무조건 `"서울 문정동"`으로 강제 채택됨.
2. **Geocoder 실행 기회 박탈 및 Context 누락**:
   - 구글 공식 안드로이드 역지오코더(`android.location.Geocoder`)보다 하드코딩된 바운딩 박스가 1순위로 배치되어, 사진의 실제 EXIF 위경도를 Geocoder에 질의할 기회가 박탈됨.
   - `DiaryViewModel.kt` 등 일부 호출부에서 `placeResolver.resolveGeoLocation(null, lat, lng)` 형태로 Context를 null로 넘겨 시스템 Geocoder가 아예 작동하지 못하고 하드코딩/폴백으로 빠지던 문제 발견.
3. **한국어 주소 체계 파싱의 불규칙성 방어 필요**:
   - Geocoder가 반환하는 `Address` 객체의 `adminArea`, `locality`, `subLocality`, `thoroughfare` 필드가 기기 제조사나 안드로이드 OS 버전에 따라 도로명 위주로 채워지거나 구(송파구) 정보가 `subLocality`에만 존재하는 등 불규칙하여, 단독 `"서울"`로 축약되거나 읍/면/동이 누락되지 않도록 견고한 파서 필요.

### 56.2. 주요 개선 및 구현 내역
1. **임의 바운딩 박스 하드코딩 전면 제거 (`PlaceResolver.kt`)**:
   - 송파구(문정동, 잠실동, 방이동 등), 강남구(역삼동 등), 영등포구 등 지리적 폴리곤을 무시하고 인접 시/군/구 오탐을 유발하던 모든 임의 위경도 사각형 바운딩 박스 코드를 전면 삭제.
2. **Android Geocoder 1순위 최우선 배치 및 지능형 한국어 행정구역 파서 구현 (`PlaceResolver.kt`)**:
   - `android.location.Geocoder`를 **1순위**로 전면 배치하여 EXIF GPS 좌표 기반 역지오코딩 수행.
   - `parseAddressToDisplayName(addr)` 엔진 구축:
     - `addr.getAddressLine(0)`의 완전한 한국어 주소 문자열과 개별 컴포넌트(`adminArea`, `locality`, `subLocality`, `thoroughfare`, `featureName`)를 복합 분석.
     - 광역 자치단체 표준 정규화: "서울특별시" -> "서울", "경기도" -> "경기", "충청북도" -> "충북", "강원도/강원특별자치도" -> "강원" 등.
     - 시/군/구 및 읍/면/동 지능형 결합:
       - 하남시 학암동/감일동 사진 -> `"경기 하남시 학암동"` 또는 `"경기 하남시"` 정확 산출.
       - 서울 마천동/문정동 사진 -> `"서울 마천동"`, `"서울 문정동"` 정확 산출.
       - 동 단위가 없는 경우에도 `"서울 송파구"`처럼 구 단위까지 완전하게 표현하여 단독 "서울" 방지.
3. **Hilt `@Inject` 생성자 구조 리팩토링 (`PlaceResolver.kt`)**:
   - `@ApplicationContext` 주입을 보장하는 단일 `@Inject` 생성자와 단위 테스트(JVM) 전용 보조 생성자를 명확히 분리하여 JVM 바이트코드 시그니처 충돌 방지 및 테스트 호환성 확보.
4. **다이어리 데이터베이스 자가 치유(Self-Healing) 연동 (`DiaryViewModel.kt`, `DiaryRepositoryImpl.kt`)**:
   - `DiaryViewModel`의 `appContext`를 Geocoder에 확실히 바인딩.
   - `cleanDuplicates()` 및 `checkAndUpgradeLegacyEntries`에 `hasCorruptedMunjeong` 자가 치유 조건 추가.
   - 기존 DB에 "서울 문정동"으로 왜곡 적재된 레거시 스텝과 타이틀을 감지하면, 스텝의 실제 GPS 위경도로 Geocoder를 재수행하여 올바른 행정구역("경기 하남시" 등)으로 자동 영속 정제.

### 56.3. 단위 테스트 및 실기기 검증
- **단위 테스트 100% 통과**:
  - `DailyRouteAggregatorTest`: 하남시 메타데이터(`placeName = "경기 하남시"`) 반영 및 골프/일상 복합 타이틀 정제 검증 전체 통과 (`BUILD SUCCESSFUL`).
- **실기기(에뮬레이터) 검증 완료**:
